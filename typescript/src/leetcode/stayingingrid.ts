export default class StayingIngrid {
   ...
   */
  public static Solver(n: number, startPos: Array<number>, s: string): Array<number> {
    // Returned variable:
   ...
   */
  public static FastSolver(n: number, startPos: Array<number>, s: string): Array<number> {
    const m: number = s.length;

    const directionObj: object = {
      U: [-1, 0],
      D: [1, 0],
      R: [0, 1],
      L: [0, -1],
    ...
    for (let t = m - 1; t >= 0; t -= 1) {
      ...
      if (futureHorizontalPosOnStepsObj[currRow + limitXMovesForDownDir] !== undefined) {
        maxNoOfInstructions = Math.min(
          maxNoOfInstructions,
          futureHorizontalPosOnStepsObj[currRow + limitXMovesForDownDir] - t - 1,
        );
      }
      if (futureHorizontalPosOnStepsObj[currRow - limitXMovesForUpDir] !== undefined) {
        maxNoOfInstructions = Math.min(
          maxNoOfInstructions,
          futureHorizontalPosOnStepsObj[currRow - limitXMovesForUpDir] - t - 1,
        );
      }
      if (futureVerticalPosOnStepsObj[currColumn + limitYMovesForRightDir] !== undefined) {
        maxNoOfInstructions = Math.min(
          maxNoOfInstructions,
          futureVerticalPosOnStepsObj[currRow + limitYMovesForRightDir] - t - 1,
        );
      }
      if (futureVerticalPosOnStepsObj[currRow - limitYMovesForLeftDir] !== undefined) {
        maxNoOfInstructions = Math.min(
          maxNoOfInstructions,
          futureVerticalPosOnStepsObj[currRow - limitYMovesForLeftDir] - t - 1,
        );
      }
