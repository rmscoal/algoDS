...
export default class SentenceSorting {
   ...
   */
  public static Solver(s: string): string {
    const sAsArray: Array<string> = s.split(' ');
    const resultAsArray: Array<string> = new Array<string>(sAsArray.length);

    for (let i = 0; i < sAsArray.length; i += 1) {
      const index: number = Number(sAsArray[i].charAt(sAsArray[i].length - 1));
